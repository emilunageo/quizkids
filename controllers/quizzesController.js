        ...
        const preguntasPromises = preguntas.map((pregunta) => {
            return new Promise((resolve, reject) => {
                ...
                db.query(preguntaQuery, [id_quiz, pregunta.pregunta], (err, result) => {
                    ...
                    const respuestas = [
                        { contenido: pregunta.respuestas[0], valor: pregunta.respuestaCorrecta === '1' ? 1 : 0 },
                        { contenido: pregunta.respuestas[1], valor: pregunta.respuestaCorrecta === '2' ? 1 : 0 },
                        { contenido: pregunta.respuestas[2], valor: pregunta.respuestaCorrecta === '3' ? 1 : 0 }
                    ];

                    const respuestaQuery = `INSERT INTO Respuestas (id_pregunta, contenido, valor) VALUES ?`;
                    const values = respuestas.map(r => [id_pregunta, r.contenido, r.valor]);
